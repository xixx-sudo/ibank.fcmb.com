...
function WebForm_DoCallback(
  ...
  var postData =
    __theFormPostData +
    "__CALLBACKID=" +
    WebForm_EncodeCallback(eventTarget) +
    "&__CALLBACKPARAM=" +
...
function WebForm_ExecuteCallback(callbackObject) {
  ...
  if (response.charAt(0) == "s") {
    if (
      typeof callbackObject.eventCallback != "undefined" &&
      callbackObject.eventCallback != null
    ) {
    ...
    if (
      typeof callbackObject.errorCallback != "undefined" &&
      callbackObject.errorCallback != null
    ) {
    ...
    if (separatorIndex != -1) {
      var validationFieldLength = parseInt(
        response.substring(0, separatorIndex)
      );
      if (!isNaN(validationFieldLength)) {
        var validationField = response.substring(
          separatorIndex + 1,
...
function WebForm_InitCallbackAddField(name, value) {
  ...
  __theFormPostData +=
    WebForm_EncodeCallback(name) + "=" + WebForm_EncodeCallback(value) + "&";
}
...
function WebForm_RestoreScrollPosition() {
  if (__nonMSDOMBrowser) {
    ...
    window.scrollTo(
      theForm.__SCROLLPOSITIONX.value,
...
function WebForm_RemoveClassName(element, className) {
  ...
  var index = currentClassName.indexOf(" " + className + " ");
  if (index >= 0) {
    element.className = WebForm_TrimString(
      currentClassName.substring(0, index) +
        " " +
